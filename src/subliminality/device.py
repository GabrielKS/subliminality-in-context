...
def seed_everything(seed: int) -> None:
    """Seed Python, NumPy, and torch RNGs (incl. the active CUDA/MPS backend).

    Backend seeding is device-aware: ``torch.manual_seed`` covers CPU/CUDA, and the
    MPS generator is seeded separately when available.
    """
    random.seed(seed)
    np.random.seed(seed)
